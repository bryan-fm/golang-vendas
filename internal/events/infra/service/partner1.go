...
func (p *Partner1) MakeReservation(req *ReservationRequest) ([]ReservationResponse, err) {
    ...
    client:= &http.Client{}
    httpResp, err := cliente.do(httpReq)
    ...
    if httpResp.StatusCode != http.StatusCreated {
        return nul, fmt.Errorf("unexpected status code: %d", httpRep.StatusCode)
    }
